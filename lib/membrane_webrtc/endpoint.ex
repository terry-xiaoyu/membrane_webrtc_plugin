defmodule Membrane.WebRTC.Endpoint do
  ...
  """
  use Membrane.Bin
  alias Membrane.{H264, RemoteStream, VP8, Pad}
  ...
  def handle_init(_ctx, opts) do
    ...
    spec = child(:webrtc, struct(ExWebRTCEndpoint, opts))

    :ok = Membrane.WebRTC.Utils.validate_signaling!(opts.signaling)

    state =
      %{tracks: %{}, payload_rtp: opts.payload_rtp, negotiated_video_codecs: nil, awaiting_pads: MapSet.new()}
      |> Map.merge(opts)
  ...
  @impl true
  def handle_element_end_of_stream(name, pad, _ctx, state) do
    Membrane.Logger.warning("Element #{inspect(name)} ended its stream, pad: #{inspect(pad)}")
    {[], state}
  end
